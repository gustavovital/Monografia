# Script com o objetivo de criar valores referentes às frequencias relativas por palavras 
...
# Cria-se uma matriz divisora e divide os valores pelos periodos

data_tibble <- readRDS('Datas/data_tibble.rds')

divisor <- matrix(ncol = 200,
                  nrow = nrow(data_tibble),
                  data = 0)

for (i in 1:nrow(data_tibble)) {
  geral <- tibble(analise = i,
                  corpus =
                    data_tibble$corpus[[i]]) %>%
  ...
  divisor[i, 1:ncol(divisor)] <- sum(geral$n)
  
}

frequencias_economicas <- readRDS('Datas/matrizdefrequencias.rds')

relativa <- frequencias_economicas / divisor
...
relativa <- data.frame(datas, relativa)

saveRDS(relativa, 'Datas/matrizdefrequenciasrelativas.rds')
rm(list = ls())
